...
int main(int argc, char** argv) {
    if(argc<2)
        ...
        if(argc==2 || strcmp(argv[2], "-r") == 0) { // Forward Replacement Arithmetic
            ...
            r = new RMD({0,2,30},50000); //Generating the set of the RMD(1,3-inf) codewords
        } else {
            w = new WordBasedText(ifname);  // Pre-process the text
            enc = new ArithmeticEncoderForward(w,bout); // Create the arithmetic encoder
            r = new RMD({0,3,30},50000); //Generating the set of the RMD(1-3,5-inf) codewords
        }
        ...
        bout.finish();
        w->output_dic("sorted_dic_"+ifname); // Print the sorted dictionary to the file
