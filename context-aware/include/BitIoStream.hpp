...
class BitInputStream final {
	...
	// Reads numbers in binary representation by length
	int readNum(int length);

	// Reads numbers in binary representation by length.
	// If any bit is EOF returns -1.
	int readNumEOF(int length);
...
class BitOutputStream final {
	...
	// Number of accumulated byts.
	public: uint64_t totalBytes;
	...
	// Writes numbers in binary representation with constant length
	void writeNum(int num, int length);
